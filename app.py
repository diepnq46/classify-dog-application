...
# Mở và đọc file TXT
with open("./static/labels/labels.txt", "r") as file:
    # Đọc từng dòng và lưu vào list (loại bỏ ký tự xuống dòng)
    breed_list = [line.strip() for line in file]

model = None


def getModel():
    loaded_model = load_model('./static/model/baseline_model.keras')
